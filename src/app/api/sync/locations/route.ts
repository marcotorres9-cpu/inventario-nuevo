...
// PUT - save all locations (full replace from client)
...
export async function POST() { return PUT(await new Request('https://placeholder', { method: 'PUT', body: '[]', headers: { 'Content-Type': 'application/json' } })); }
export async function DELETE() { return NextResponse.json({ success: true }, { headers: { 'Cache-Control': 'no-store', 'Access-Control-Allow-Origin': '*' } }); }
