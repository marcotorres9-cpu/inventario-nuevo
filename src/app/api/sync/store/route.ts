...
export async function GET() {
  try {
    const rows = await query('SELECT * FROM "Store" WHERE id = $1', ['main']);
    if (rows.length > 0) {
      const r = rows[0];
    ...
    // Fallback to store_config
    const rows2 = await query("SELECT * FROM store_config WHERE id='main'");
    if (rows2.length > 0) {
      const r = rows2[0];
      const parse = (v: string) => { try { return JSON.parse(v); } catch { return v; } };
      return NextResponse.json({
        id: r.id, name: r.name || 'Mi Tienda', address: r.address || '',
        phone: r.phone || '', email: r.email || '', currency: r.currency || 'MXN',
        footer: r.footer || r.footer_text || '', logo: r.logo || null,
        categories: parse(r.categories), brands: parse(r.brands), colors: parse(r.colors),
    ...
    try {
      await query(
        ...
        [b.name||b.storeName||'Mi Tienda', b.address||'', b.phone||'', b.email||'', b.website||'', b.logo||null,
         b.currency||'MXN', b.footer||b.footerText||'',
