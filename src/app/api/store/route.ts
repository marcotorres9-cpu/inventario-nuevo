...
export async function OPTIONS(){
  return new NextResponse(null,{status:204,headers:{...H,'Access-Control-Allow-Methods':'GET,PUT,POST,OPTIONS','Access-Control-Allow-Headers':'Content-Type,Authorization'}});
}
...
export async function GET(){
  try{
    ...
    console.error('[DB] GET /store:',e.message);
    return NextResponse.json({},{headers:H});
  }
}
...
export async function PUT(request:Request){
  try{
    ...
    console.error('[DB] PUT /store:',e.message);
